...
public:
  Drawable(const std::string& n, const Vector2f& pos, const Vector2f& vel): 
    name(n), position(pos), velocity(vel), initialPosition(position), initialVelocity(velocity), scale(1.0) { }

  Drawable(const Drawable& s) : 
    name(s.name), position(s.position), velocity(s.velocity), initialPosition(s.initialPosition), initialVelocity(s.initialVelocity), scale(s.scale)
    { }
  ...
  void reset() { position = initialPosition; velocity = initialVelocity; }
private:
  std::string name;
  Vector2f position;
  Vector2f velocity;
